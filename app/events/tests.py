...
class BoardingEventAPITest(APITestCase):
    ...
    def setUp(self):
        self.school = School.objects.create(name="Test School")
        self.bus = Bus.objects.create(license_plate="TEST-001", capacity=50)
        self.kiosk = Kiosk.objects.create(
            bus=self.bus, kiosk_id="KIOSK001", api_key_hash="test_kiosk_key_123"
        )
        self.student = Student.objects.create(
            school=self.school,
            name="Test Student",
    ...
    def test_create_boarding_event(self):
        ...
        response = self.client.post(
            ...
            data,
            format="json",
            HTTP_X_KIOSK_API_KEY=self.kiosk.api_key_hash,
        )
