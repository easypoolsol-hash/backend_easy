...
class DeviceLogModelTest(TestCase):
    ...
    def test_device_log_string_representation(self):
        ...
        log = DeviceLogFactory(
            kiosk=self.kiosk, log_level="ERROR", message="Error occurred"
        )
...
class KioskSerializerTest(TestCase):
    ...
    def setUp(self):
        ...
        self.kiosk = KioskFactory(
            bus=self.bus, firmware_version="1.0.0", battery_level=85.5
        )
...
class KioskAPITest(APITestCase):
    ...
    def setUp(self):
        ...
        self.bus = BusFactory()
        self.kiosk = KioskFactory(
            bus=self.bus, api_key_hash="hashed_key_123", firmware_version="1.0.0"
        )
    ...
    def test_kiosk_heartbeat(self):
        ...
        response = self.client.post(
            url, data, format="json", HTTP_X_KIOSK_API_KEY=self.kiosk.api_key_hash
        )
    ...
    def test_kiosk_heartbeat_invalid_kiosk(self):
        ...
        # Add invalid API key
        response = self.client.post(
            url, data, format="json", HTTP_X_KIOSK_API_KEY="invalid_key"
        )
    ...
    def test_kiosk_log_invalid_kiosk(self):
        ...
        data = {
            "kiosk_id": "INVALID-KIOSK",
            "logs": [{"log_level": "ERROR", "message": "Test error"}],
        }
        # Add invalid API key
        response = self.client.post(
            url, data, format="json", HTTP_X_KIOSK_API_KEY="invalid_key"
...
class DeviceLogAPITest(APITestCase):
    ...
    def setUp(self):
        ...
        self.bus = BusFactory()
        self.kiosk = KioskFactory(bus=self.bus)
        self.log = DeviceLogFactory(
            kiosk=self.kiosk, log_level="INFO", message="Test log"
        )
