        ...
        for field_name, field_schema in schema["properties"].items():
            ...
            if is_nullable:
                ...
                if schema_name == "Student":
                    print(f"[HOOK] Student.{field_name} is nullable")

        # Remove nullable fields from required array
        if nullable_fields:
            if schema_name == "Student":
                print(f"[HOOK] Student required before: {schema['required']}")
                print(f"[HOOK] Removing nullable fields: {nullable_fields}")

            schema["required"] = [field for field in schema["required"] if field not in nullable_fields]

            if schema_name == "Student":
                print(f"[HOOK] Student required after: {schema['required']}")
